        ...
        <>
            ...
            <div className={`w-64 bg-white border-r border-gray-100 fixed inset-y-0 left-0 flex flex-col z-50 shadow-2xl md:shadow-none transform transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0`}>
                ...
                <nav className="flex-1 px-4 space-y-2 mt-2">
                    <Link
                        to="/"
                        onClick={() => setIsOpen(false)} // To close mobile menu on clicking the link
                        className={`flex items-center space-x-3 px-4 py-3 rounded-xl text-sm font-bold transition-all duration-200 ${location.pathname === '/'
                    ...
                    <Link
                        to="/rules"
                        onClick={() => setIsOpen(false)}
                        className={`flex items-center space-x-3 px-4 py-3 rounded-xl text-sm font-bold transition-all duration-200 ${location.pathname === '/rules'
                                ? 'bg-blue-50 text-blue-600 shadow-sm'
                                : 'text-gray-500 hover:bg-gray-50 hover:text-gray-800'
                            }`}
                    >
